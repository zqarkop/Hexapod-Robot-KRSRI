...
namespace ping {
  ...
  bool isOnSRWR = false;
  bool isOnSLWR = false;
  unsigned int jarakC = 0;
  unsigned int jarakA = 0;
  unsigned int jarakE = 0;
  unsigned int jarakPingC = 0;
  unsigned int jarakPingA = 0;
  unsigned int jarakPingE = 0;
  unsigned int jarakPingCandi = 0;
  unsigned int jarakPingAandi = 0;
  ...
  unsigned int read_pingC (int trigPin, int echoPin) {
  ...
  unsigned int read_pingCandi (int trigPin, int echoPin) {
    // ping only waits for pulse in for 5ms
    // more than that, will return integer 64
  ...
  void updateC () {
    ...
    switch (state_nextPingSensor) {
      case PING_A:
        currentPingValue = read_pingC(ULTRA_A_TRIG, ULTRA_A_ECHO);
        near_a = currentPingValue < (9 + offset);
        far_a = currentPingValue < (8 + offsetFar);
        jarakPingA = currentPingValue;
        isOnSRWR = currentPingValue < (14 + offset);
        state_nextPingSensor = PING_C;
        break;
      case PING_C:
        currentPingValue = read_pingC(ULTRA_C_TRIG, ULTRA_C_ECHO);
        near_c = currentPingValue < (12 + offset);
        far_c = currentPingValue < (8 + offsetFar);
        jarakPingC = currentPingValue;
        state_nextPingSensor = PING_E;
        break;
      case PING_E:
        currentPingValue = read_pingC(ULTRA_E_TRIG, ULTRA_E_ECHO);
        near_c = currentPingValue < (12 + offset);
        far_c = currentPingValue < (8 + offsetFar);
        jarakPingE = currentPingValue;
  ...
  void updateJarak () {
    ...
    switch (state_nextPingSensor) {
      case PING_A:
        currentPingValue = read_pingCandi(ULTRA_A_TRIG, ULTRA_A_ECHO);
        near_a = currentPingValue < (9 + offset);
        far_a = currentPingValue < (8 + offsetFar);
        jarakPingAandi = currentPingValue;
        isOnSRWR = currentPingValue < (14 + offset);
        state_nextPingSensor = PING_C;
        break;
      case PING_C:
        currentPingValue = read_pingCandi(ULTRA_C_TRIG, ULTRA_C_ECHO);
        near_c = currentPingValue < (12 + offset);
        far_c = currentPingValue < (8 + offsetFar);
        jarakPingCandi = currentPingValue;
        state_nextPingSensor = PING_A;
        break;
    }
  }
